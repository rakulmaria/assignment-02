...
public class Queries
{
    ...
    {
        return wizards.Where(w => w.Creator.Contains("J.K. Rowling")).Select(w => w.Name);
    }
    ...
    {
        return wizards.First(w => w.Name.Contains("Darth")).Year;
    }
    ...
    {
        return wizards.GroupBy(w => w.Creator).OrderByDescending(w => w.Key).Select(w => (w.Key, w.Select(x => x.Name)));
    }
}